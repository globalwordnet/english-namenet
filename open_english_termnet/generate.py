...
import pickle
import argparse
from open_english_termnet import WIKIDATA_DB, WORDNET_SOURCE, load_wordnet_data, fetch_in_chunks, read_wikidata_with_prop_vals, get_labels_and_defn, oewn_extract, wikidata_extract
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", type=str, help="Output file path", default="oenn")
    ...
    if not args.skip_overlaps:
        ...
        for wn_hyp, wds in tqdm(overlaps_by_oewn.items(), desc="Processing OEWN synsets", position=0):
            ...
            if "," in lemma:
                lemma = lemma.split(",")[0]
            with open(f"{args.output_folder}/noun.{lemma}.yaml", "w") as f:
                for wd in wds:
    ...
    if not args.skip_humans:
        ...
        with open(f"{args.output_folder}/noun.human.yaml", "w") as f:
    ...
    if not args.skip_taxons:
        ...
        with open(f"{args.output_folder}/noun.taxon_working.csv", "w") as f:
            writer = csv.writer(f)
        ...
        with open(f"{args.output_folder}/noun.taxon.yaml", "w") as f:
            with open(f"{args.output_folder}/noun.species.yaml", "w") as f_species:
                csv_line_count = sum(1 for line in open(f"{args.output_folder}/noun.taxon_working.csv"))
                with open(f"{args.output_folder}/noun.taxon_working.csv", "r") as f_csv:
                    ...
                    for row in tqdm(reader, desc="Writing taxons", total=csv_line_count):
                        ...
                        if " " in sci_name:
                            new_id, entry = make_entry(entity, cursor, wn_hyps, wd2entry,
                                                       inst=False)
                            if entry:
                                yaml.dump({new_id: entry}, f_species, sort_keys=False)
        ...
        os.remove(f"{args.output_folder}/noun.taxon_working.csv")
